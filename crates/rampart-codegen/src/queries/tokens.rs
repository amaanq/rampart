...
#[derive(Debug, Clone, PartialEq, Copy, serde::Serialize)]
pub struct PasswordResetFailure {
    pub used: bool,
    pub expired: bool,
}
...
#[derive(Debug, Clone, PartialEq, Copy, serde::Serialize)]
pub struct EmailChangeFailure {
    pub used: bool,
    pub expired: bool,
}
#[derive(Debug, Clone, PartialEq, Copy, serde::Serialize)]
pub struct MailboxVerifyFailure {
    pub used: bool,
    pub expired: bool,
}
...
pub struct PasswordResetFailureQuery<'c, 'a, 's, C: GenericClient, T, const N: usize> {
    client: &'c C,
    params: [&'a (dyn postgres_types::ToSql + Sync); N],
    query: &'static str,
    cached: Option<&'s tokio_postgres::Statement>,
    extractor: fn(&tokio_postgres::Row) -> Result<PasswordResetFailure, tokio_postgres::Error>,
    mapper: fn(PasswordResetFailure) -> T,
}
impl<'c, 'a, 's, C, T: 'c, const N: usize> PasswordResetFailureQuery<'c, 'a, 's, C, T, N>
where
    C: GenericClient,
{
    pub fn map<R>(
        self,
        mapper: fn(PasswordResetFailure) -> R,
    ) -> PasswordResetFailureQuery<'c, 'a, 's, C, R, N> {
        PasswordResetFailureQuery {
            client: self.client,
            params: self.params,
            query: self.query,
            cached: self.cached,
...
pub struct EmailChangeFailureQuery<'c, 'a, 's, C: GenericClient, T, const N: usize> {
    client: &'c C,
    params: [&'a (dyn postgres_types::ToSql + Sync); N],
    query: &'static str,
    cached: Option<&'s tokio_postgres::Statement>,
    extractor: fn(&tokio_postgres::Row) -> Result<EmailChangeFailure, tokio_postgres::Error>,
    mapper: fn(EmailChangeFailure) -> T,
}
impl<'c, 'a, 's, C, T: 'c, const N: usize> EmailChangeFailureQuery<'c, 'a, 's, C, T, N>
where
    C: GenericClient,
{
    pub fn map<R>(
        self,
        mapper: fn(EmailChangeFailure) -> R,
    ) -> EmailChangeFailureQuery<'c, 'a, 's, C, R, N> {
        EmailChangeFailureQuery {
            client: self.client,
            params: self.params,
            query: self.query,
            cached: self.cached,
...
pub struct MailboxVerifyFailureQuery<'c, 'a, 's, C: GenericClient, T, const N: usize> {
    client: &'c C,
    params: [&'a (dyn postgres_types::ToSql + Sync); N],
    query: &'static str,
    cached: Option<&'s tokio_postgres::Statement>,
    extractor: fn(&tokio_postgres::Row) -> Result<MailboxVerifyFailure, tokio_postgres::Error>,
    mapper: fn(MailboxVerifyFailure) -> T,
}
impl<'c, 'a, 's, C, T: 'c, const N: usize> MailboxVerifyFailureQuery<'c, 'a, 's, C, T, N>
where
    C: GenericClient,
{
    pub fn map<R>(
        self,
        mapper: fn(MailboxVerifyFailure) -> R,
    ) -> MailboxVerifyFailureQuery<'c, 'a, 's, C, R, N> {
        MailboxVerifyFailureQuery {
            client: self.client,
            params: self.params,
            query: self.query,
            cached: self.cached,
...
pub struct PasswordResetFailureStmt(&'static str, Option<tokio_postgres::Statement>);
pub fn password_reset_failure() -> PasswordResetFailureStmt {
    PasswordResetFailureStmt(
        "SELECT used_at IS NOT NULL AS used, expires_at <= now() AS expired FROM password_reset_token WHERE token_hash = $1",
        None,
    )
}
impl PasswordResetFailureStmt {
    pub async fn prepare<'a, C: GenericClient>(
        mut self,
        client: &'a C,
    ) -> Result<Self, tokio_postgres::Error> {
    ...
    pub fn bind<'c, 'a, 's, C: GenericClient, T1: crate::BytesSql>(
        ...
        client: &'c C,
        token_hash: &'a T1,
    ) -> PasswordResetFailureQuery<'c, 'a, 's, C, PasswordResetFailure, 1> {
        PasswordResetFailureQuery {
            client,
            params: [token_hash],
            query: self.0,
            cached: self.1.as_ref(),
            extractor:
                |row: &tokio_postgres::Row| -> Result<PasswordResetFailure, tokio_postgres::Error> {
                    Ok(PasswordResetFailure {
                        used: row.try_get(0)?,
                        expired: row.try_get(1)?,
                    })
                },
            mapper: |it| PasswordResetFailure::from(it),
        }
    }
}
...
pub struct EmailChangeFailureStmt(&'static str, Option<tokio_postgres::Statement>);
pub fn email_change_failure() -> EmailChangeFailureStmt {
    EmailChangeFailureStmt(
        "SELECT used_at IS NOT NULL AS used, expires_at <= now() AS expired FROM email_change_token WHERE token_hash = $1",
        None,
    )
}
impl EmailChangeFailureStmt {
    pub async fn prepare<'a, C: GenericClient>(
        mut self,
        client: &'a C,
    ) -> Result<Self, tokio_postgres::Error> {
    ...
    pub fn bind<'c, 'a, 's, C: GenericClient, T1: crate::BytesSql>(
        ...
        client: &'c C,
        token_hash: &'a T1,
    ) -> EmailChangeFailureQuery<'c, 'a, 's, C, EmailChangeFailure, 1> {
        EmailChangeFailureQuery {
            client,
            params: [token_hash],
            query: self.0,
            cached: self.1.as_ref(),
            extractor:
                |row: &tokio_postgres::Row| -> Result<EmailChangeFailure, tokio_postgres::Error> {
                    Ok(EmailChangeFailure {
                        used: row.try_get(0)?,
                        expired: row.try_get(1)?,
                    })
                },
            mapper: |it| EmailChangeFailure::from(it),
        }
    }
}
...
pub struct MailboxVerifyFailureStmt(&'static str, Option<tokio_postgres::Statement>);
pub fn mailbox_verify_failure() -> MailboxVerifyFailureStmt {
    MailboxVerifyFailureStmt(
        "SELECT used_at IS NOT NULL AS used, expires_at <= now() AS expired FROM mailbox_verify_token WHERE token_hash = $1",
        None,
    )
}
impl MailboxVerifyFailureStmt {
    pub async fn prepare<'a, C: GenericClient>(
        mut self,
        client: &'a C,
    ) -> Result<Self, tokio_postgres::Error> {
    ...
    pub fn bind<'c, 'a, 's, C: GenericClient, T1: crate::BytesSql>(
        ...
        client: &'c C,
        token_hash: &'a T1,
    ) -> MailboxVerifyFailureQuery<'c, 'a, 's, C, MailboxVerifyFailure, 1> {
        MailboxVerifyFailureQuery {
            client,
            params: [token_hash],
            query: self.0,
            cached: self.1.as_ref(),
            extractor:
                |row: &tokio_postgres::Row| -> Result<MailboxVerifyFailure, tokio_postgres::Error> {
                    Ok(MailboxVerifyFailure {
                        used: row.try_get(0)?,
                        expired: row.try_get(1)?,
                    })
                },
            mapper: |it| MailboxVerifyFailure::from(it),
        }
    }
}
