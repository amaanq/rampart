...
fn mock_domain_setup(id: i64) -> DomainSetup {
   ...
   let dkim = vec![
      DkimRecord {
         ...
         value:     "v=DKIM1; k=ed25519; h=sha256; p=11qYAYKxCrfVS/7TyWwQ1Y8GfH4v7sQxY8P0vR8zJtE="
            .into(),
      },
      DkimRecord {
         algorithm: "rsa".into(),
         ...
         value:     "v=DKIM1; k=rsa; h=sha256; \
                     p=MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8Apreviewkeythatwrapsacrossalongdnsvalue"
            .into(),
      },
   ];
   ...
   let status: DnsStatus = base
      ...
      .map(|record| {
         ...
         (record.id.clone(), DnsObservation {
