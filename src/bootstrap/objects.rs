...
pub(super) async fn patch_outbound_strategy(
   ...
   if unchanged && cur.is_some() {
      stats.skipped += 1;
      tracing::info!("MtaOutboundStrategy: canonical rampart branches already in place, skipping");
      return Ok(());
   }
...
pub(super) async fn upsert_notifier(
   ...
   // hit by domainId. List<Credential> uses the same indexed-object shape as
   // `match`.
   ...
   for candidate_id in &candidates {
         ...
         && obj.get("domainId").and_then(|v| v.as_str()) == Some(domain_id.as_str())
      {
         ...
         if dry_run {
            println!(
               "DRY RUN: Account/set update {candidate_id} (password redacted): \
                {{\"credentials\":[{{...}}]}}"
            );
            stats.patched += 1;
            return Ok(());
   ...
   if dry_run {
      println!(
         "DRY RUN: Account/set create (password redacted): \
          {{\"name\":\"{local}\",\"domainId\":\"{domain_id}\"}}"
      );
      return Ok(());
   }
