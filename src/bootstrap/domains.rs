//! Alias-domain marker logic — `Domain.description` distinguishes rampart-managed
//! Domains from user-managed ones, so reconcile never destroys unrelated mail
//! domains.
...
pub(crate) async fn upsert_managed_alias_domain(
    ...
    if let Some(id) = client.query_by_name("Domain", name).await? {
        ...
        if current == RAMPART_DOMAIN_MARKER {
            if dry_run {
                println!(
                    "DRY RUN: would heal marker-owned Domain/{id} ({name}) to canonical state"
                );
                stats.skipped += 1;
                return Ok(());
            }
            ...
            if dkim_ids.is_empty() {
                tracing::warn!(
                    domain = name,
                    "marker-owned Domain has zero DkimSignatures; toggling DKIM to force key regeneration"
                );
                client
                    .set_update(
                        "Domain",
            ...
            tracing::info!(
                domain = name,
                description = current,
                "alias Domain object already exists (NOT rampart-managed); routing through it without taking ownership"
            );
        }
        return Ok(());
    }
...
pub(crate) async fn reconcile_alias_domains(
    ...
    for item in &all {
        ...
        if name.eq_ignore_ascii_case(notifier_domain) {
            bail!(
                "refusing to reconcile: alias-domain '{name}' hosts the rampart-notifier ({notifier_domain}); \
                 stalwart will reject the Domain destroy because Account has an FK and we'd already have \
                 disabled the Domain and destroyed its DKIM keys. Move the notifier to a different domain \
                 or re-add '{name}' to rampart's alias_domain table."
            );
        }
